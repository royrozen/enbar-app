...
Today every table's RLS policy is `using (true)` for `anon, authenticated` — any authenticated session can read or write any row in any table. Goals:
...
- Lock `exception_logs` once `status = 'approved'`, and prevent that transition from happening without `signed_path`, regardless of who's asking — a same-invariant-for-everyone rule, not a role split.
- No behavior change to what a legitimate user can already do through the UI — only illegitimate access (bypassing the UI) starts failing.
...
1. Stage against dev project only, one table at a time, in dependency order: `profiles` → `team_leads` → `clients`/`projects` → `reports`/`report_photos` → `exception_logs`/`exception_photos` → `part_orders`/`part_requests` → `catalog_items`.
2. After each table, manually walk both roles through every screen that touches it before moving to the next.
...
- [ ] Test matrix items 2, 3, 4, 5, 6, 8, 9 (all "must fail" cases) are rejected by the database under a real authenticated session, not just hidden by the UI.
