...
As a stopgap against casual/accidental access (there being no real auth), the entire manager area (`/manager`, `/manager/report/:id`, `/manager/settings`) is additionally gated by `AdminGate` in `App.jsx`: a single shared password (`VITE_ADMIN_PASSWORD` env var, fallback `enbar2026`), unlocked once per tab via `sessionStorage['enbar_admin_unlocked']` (shared key, so unlocking once covers the whole area). This is not real security — the password ships in the client bundle — just a speed bump. Don't mistake it for Phase-2 auth.
