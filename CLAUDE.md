...
**Real authentication via Supabase phone OTP.** `src/pages/Login.jsx` collects an Israeli mobile number, sends an OTP (`sendOtp`/`verifyOtp` in `src/lib/auth.js`, wrapping `supabase.auth.signInWithOtp`/`verifyOtp`), and `src/lib/AuthContext.jsx` exposes the resulting `session` + `profile` (fetched from a `profiles` table, `role` one of `team_lead` / `factory_manager` / `platform_admin`) via `useAuth()`. Route guards in `src/App.jsx` are three `Require*` wrapper components — `RequireProfile` (any signed-in profile), `RequireManager` (not `team_lead`), `RequireFactoryManager` (`factory_manager` or `platform_admin` only, gates `/manager/settings`) — mirror these rather than inventing a new pattern. The earlier Phase-1 no-auth design (localStorage profile picker at `src/pages/ProfilePicker.jsx`, a shared-password `AdminGate`) has been fully replaced; those files/mechanisms no longer exist in the codebase. `src/lib/profile.js` (the old `PROFILES` map / `enbar_profile` localStorage helper) is leftover and no longer wired into `App.jsx`.

**Routing** (`src/App.jsx`): `/` (login) → `/home` (team lead) → `/report/new`, `/report/:id` (team lead's own reports) → `/manager` (dashboard) → `/manager/report/:id` → `/manager/settings` (admin, factory manager / platform admin only).
