...
| 2026-09-15 | Maintenance checklist persists only on submit; already-checked tasks lock for the rest of the week | A localStorage draft of unsubmitted checkboxes was built and rejected — half-filled state that never reached the DB read as progress that wasn't recorded. Completed work is shared shop-floor state, so it comes from the DB (`machine_week_checked_tasks()`) and is visible to every worker, not just whoever checked it |
